...
secrets    обёртка над keyring: свой набор секретов на каждый каталог данных; в тестах память
probe      что сервер говорит о себе: CAPABILITY, LIST с атрибутами и ролями, EXAMINE каждой папки
```

В `provider/wire.rs` лежит запись разговора с IMAP-сервером (`mailctl --wire FILE`). Она стоит над TLS, маскирует
аргументы LOGIN и AUTHENTICATE, строку токена после них и их литералы, а тела писем режет до 200 байт. Файл создаётся
с правами 0600.
...
16. **Первый настоящий ящик.** Роль папки в первую очередь берётся из атрибутов LIST (RFC 6154, на любом языке
   интерфейса), имя служит только подсказкой: «Вся почта» и «Удалённые» тоже узнаются. Каждая роль достаётся одной
   выбираемой папке, INBOX всегда Входящие. Имя для показа декодируется из modified UTF-7, без префикса сервера
   (`[Gmail]/`, `INBOX.`), а уровни разделены ` / `. Если Archive или Delete некуда положить письмо, ядро отвечает
   `NoFolder`. На своём сервере приложение предлагает создать папку: CREATE с `USE (\Archive)`, когда сервер умеет
   CREATE-SPECIAL-USE, и внутри `INBOX.`, если все папки живут там. У Gmail папка есть, но скрыта от IMAP, поэтому
   приложение говорит, где включить «Show in IMAP». На Gmail роли по именам не угадываются, ярлык «Archives»
   остаётся ярлыком. Секреты привязаны к каталогу данных: его профиль записан в нём самом (`secrets-profile`)
   и переезжает вместе с ним, как контейнер приложения на iOS при обновлении. Каталог, где почта была до
   профилей, остаётся на общем сервисе, новый получает свой, так что `mailctl --data-dir` не читает, не
   перезаписывает и не удаляет пароль приложения. SMTP представляется
   как `[127.0.0.1]` и не выдаёт имя компьютера. Проверка реального ящика: `mailctl probe`, `check-smtp`,
   `add-imap` с проверкой входа и `folders`; порядок шагов в `docs/SMOKE.md`.

17. **Читается и не вредит.**
   - *Письма со своими цветами.* Ядро сообщает `styled` (`has_own_colours`), и такое письмо показывается на белой
     «бумаге» с тёмным текстом и синими ссылками, в любой теме. Цвета `<body>` (bgcolor, text, style) переносятся на
     обёртку. Их значения проходят строгую проверку: только hex, ключевые слова, `rgb()`. Из `background` остаётся
     только цвет. `bgcolor` рисуется как фон. Тёмная заливка без своего цвета текста получает белый текст.
   - *Встроенные картинки.* Размер берётся из заголовка файла, без декодирования: у GIF с учётом всех кадров, у JPEG
     без маркеров без длины. Больше 8192 px по стороне или 25 Мп картинка не встраивается. Декодируется не шире
     колонки.
   - *Вложения.* Имя файла чистится: без bidi- и невидимых символов, без разделителей строк, без имён устройств
     Windows (включая COM¹ и CONOUT$). Файлы, которые могут запустить код или открыть браузер, никогда не
     открываются из письма: список в `files::risky_extension`, решение по имени, не по MIME. Их можно только
     сохранить (на телефоне поделиться) после явного вопроса. Всё записанное с диска помечается как скачанное
     из интернета: com.apple.quarantine на macOS, Zone.Identifier на Windows. В чипе расширение всегда видно,
     длинное имя сокращается посередине, по графемам.
   - *Кто написал.* `Address::display` чистит имя от bidi-, невидимых и управляющих символов и схлопывает пробелы.
     Имя, похожее на адрес (`@`, `<`, `>`), не показывается: вместо него виден настоящий адрес. Подсказка быстрого
     ответа называет адресата по правилу ядра: отправитель последнего письма, а если последнее письмо своё
     (`is_mine`), то его получатели.

18. **Старая почта и поиск на сервере.** Синк папки покрывает UID от её «пола» (`floor_uid`) и выше; первый
   синк берёт окно из 200 последних писем. «Load older mail» внизу списка берёт по 200 следующих писем
   ниже пола каждой папки, которую показывает список, и опускает пол: дальше синк следит и за ними. «Search
   on the server» переводит запрос в IMAP SEARCH (TEXT, FROM, TO, SUBJECT, BEFORE, SINCE; не-ASCII —
   литералом LITERAL+ или строкой после CHARSET UTF-8). Запрос без слов, отправителя, темы и дат на
   сервер не уходит. Ищет в All Mail на Gmail, в остальных случаях во всех синхронизированных папках,
   кроме Spam, Trash и Drafts. Найденное ложится в кэш, а пол не двигается: следующий синк не качает
   годы почты между находкой и окном, но флаги и удаление найденного писем отслеживает.

19. **Любая ширина окна.** От 1100 px видны сайдбар, список и чтение. Ниже сайдбар открывается кнопкой слева
   в верхней строке как выдвижная панель; выбор папки её закрывает. Поле поиска сжимается вместе с окном. Тест
   проходит по ширинам от 360 до 1600 px и падает при любом переполнении.

20. **Новая почта, когда Flomsi не на экране.** После синка, который что-то скачал, непрочитанные треды
   Входящих сравниваются с уже известными. О новых (или о треде с новым письмом) сообщает системное
   уведомление, по одному на тред, при больше чем трёх — сводка. Нажатие открывает тред. Пока приложение
   на экране, уведомлений нет. На macOS закрытое окно лишь прячется: Flomsi остаётся в Dock, продолжает
   синк и уведомляет, клик по иконке или уведомлению возвращает окно, ⌘Q завершает.

21. **Вход через провайдера.** Google и Microsoft входят по OAuth 2.0 с PKCE, без пароля. На компьютере страница
   входа открывается в браузере, ответ ловит loopback-адрес (`127.0.0.1` для Google, `localhost` для Microsoft,
   порт слушается по IPv4 и IPv6); вход можно отменить. На телефоне страницу показывает системный лист
   (AppAuth: ASWebAuthenticationSession на iOS, Custom Tabs на Android), возврат по схеме
   `com.googleusercontent.apps.<id>:/oauth2redirect` или `msal<id>://auth`. Код меняет на токены ядро (`http.rs`,
   свой минимальный HTTPS-клиент), refresh-токен уходит в keychain, access-токен обновляется за 2 минуты до конца
   под мьютексом, IMAP и SMTP входят через XOAUTH2, отказ сервера даёт одну попытку со свежим токеном. Scope
   Google — `https://mail.google.com/` (IMAP иначе не пускает). Отказы названы: `cancelled`, `denied`, `scope`,
   `no_refresh`, `admin`, `imap_off`, `duplicate_password`, `wrong_account`, `timeout` (`diagnose::sign_in`).
   «Войти снова» передаёт адрес как `login_hint` и заменяет токены того же аккаунта, не трогая его почту.
   Client ID задаются при сборке (`FLOMSI_GOOGLE_CLIENT_ID`, `…_IOS_CLIENT_ID`, `…_ANDROID_CLIENT_ID`,
   `FLOMSI_MICROSOFT_CLIENT_ID` из секретов репозитория); без них кнопки провайдера не показываются, остаются
   пароли приложений.

22. **Телефон.** Экран почты на телефоне (`features/phone/phone_shell.dart`) свой, не сжатый десктоп: сверху
   название ящика и строка «аккаунт · обновлено 14:02» (меню аккаунтов, если их несколько) и «Ещё»; снизу панель
   Mailboxes, Search, Unread, Compose. Ящики — лист снизу, действия с письмом — долгое нажатие или свайп
   (вправо архив только во Входящих, влево удаление там, где это имеет смысл). Архив, удаление и перенос ждут
   5 секунд с Undo (`phone/undo.dart`): тред прячется из списка (`hiddenThreadsProvider`), а сама операция
   идёт по таймеру, при уходе приложения в фон или при следующей операции; если приложение убито в эти секунды,
   письмо остаётся на месте. Тред, ответ и настройки открываются отдельными экранами с нижней панелью своих
   кнопок. Размеры для пальца: цели 48, строки списка от 76, чтение 16/1.55. Первый запуск (`onboarding/`):
   экран приветствия с «Continue with Google», «Sign in with Microsoft» и «Other email account» — список
   провайдеров с пошаговой инструкцией по паролю приложения. Планшеты оставляют раскладку редактора.
23. **Удалить навсегда и очистить.** Удаляется навсегда только то, что уже в Корзине или Спаме, и только после
   вопроса, без Undo. «Delete forever» сначала называет копии треда в этих папках (`Actions::bin_copies`: папка,
   UIDVALIDITY, UID), вопрос говорит, сколько их, и по «да» `Actions::delete_forever(&copies)` кладёт обычный
   `Op::Delete` только на те из них, что ещё там же: письмо, пришедшее в тред за время вопроса, остаётся.
   На сервере это `UID STORE +FLAGS (\Deleted)` и `UID EXPUNGE` (у Gmail — удаление насовсем). В Корзине Delete
   и есть «Delete forever»: дальше класть некуда. Перенос проверяет, что письмо ещё на сервере (`UID SEARCH UID`):
   MOVE несуществующего UID отвечает OK и ничего не делает. Сделанным это считается, только если так и есть:
   ответ прошлой попытки потерялся на оборванном соединении (перенос дошёл) или это архив Gmail (письмо и так не
   во Входящих). Иначе перенос снимается со словами «the message is no longer in that folder»: возврат из Корзины
   по устаревшей копии не выдаёт себя за сделанный, и следующая очистка не удалит письмо молча. «Empty Trash» / «Empty Junk» идут в три шага.
   (1) Отложенное на телефоне действие (Undo) уходит в очередь; под модальным «Checking…», когда трогать ничего
   нельзя, папка каждого аккаунта заново читается с сервера вместе с его очередью (`sync_bin`, в очереди аккаунта),
   и `Actions::check_bin` снимает снимок: UIDVALIDITY, `below` (UIDNEXT или последний UID в кэше + 1) и `seen` —
   письма, которые были в кэше. Без связи ничего не удаляется. (2) Вопрос называет аккаунты и то, что всплыло при
   чтении: например, возврат из Корзины, от которого сервер отказался (письмо снова в Корзине). (3) По «да»
   отложенное ещё раз уходит в очередь, и `Actions::empty_folder(&check)` одной транзакцией, без сети, удаляет из
   кэша письма из `seen` и кладёт `Op::Empty { below, keep }`. В `keep` — то, что выносят операции в очереди (кроме
   снятых, чьё письмо уже было на виду при проверке), всё, что появилось в папке после проверки (возврат, снятый
   фоновым синком), и «дыры» — письма, которые сервер не отдал: о них пользователя не спрашивали. Если папку за это время перестроили (другая
   UIDVALIDITY) или проверке больше 15 минут, очистка отказывает. Сервер удаляет `1:below-1` без `keep` и без того,
   что выносят операции, поставленные позже: старые письма, которых нет в кэше, уходят; пришедшее после проверки и
   то, что пользователь вернул, остаются. Роль Корзины или Спама по имени угадывается только наверху дерева или
   сразу под INBOX / `[Gmail]` («Clients/Acme/Junk» — папка пользователя: туда не попадает ни Delete, ни Empty);
   атрибуту SPECIAL-USE верим на любой глубине. Роль проверяется и перед отправкой, без UIDVALIDITY ничего не
   удаляется. Удаляющие команды провайдер шлёт только в папку, открытую сейчас (`open_for_deleting`): SELECT
   сначала забывает прошлую папку, и неудачный SELECT (например, с `* NO [ALERT]`) не оставляет удаление в чужой
   папке. Пока операция ждёт, синк, «Load older mail» и поиск на сервере не возвращают письма ниже `below`
   (`Waiting::leaving`, кроме `keep`).
...
folders.delimiter                  -- v10: разделитель уровней с сервера, для показа имени
folders.floor_uid                  -- v11: самый старый UID, который покрывает синк
...
| телефон | `PhoneShell`: список, панели сверху и снизу, экраны треда, письма и настроек |
...
`core` (fmt, clippy, tests, mailctl для Linux), `app-check` (analyze, tests), `macos` (release, образ `Flomsi-macOS.dmg` из `scripts/make_dmg.sh`),
