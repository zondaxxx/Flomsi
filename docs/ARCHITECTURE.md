...
1. **Local-first.** Любое действие сначала применяется к локальной базе и пишется в `outbox`, одной транзакцией
   (`Store::batch`): синк видит либо и изменение, и операцию, либо ничего. Синк проигрывает outbox на сервер.
   Пока операция ждёт, её копии защищены: флаги с сервера для них не пишутся, а перемещённое письмо не сохраняется
   заново; это проверяется в момент записи (`apply_server_flags`, `upsert_fetched`), а не по снимку начала синка.
   С CONDSTORE отложенное изменение не сдвигает HIGHESTMODSEQ, так что сервер пришлёт его снова. Ответы SEARCH,
   LIST и FETCH читает свой разборщик (`checked` в `provider/imap.rs`): NO/BAD — ошибка, оборванный поток — обрыв;
   SEARCH, вернувший меньше писем, чем насчитал SELECT, повторяется: удаляет только ответ, который совпал дважды.
   NO на пачку FETCH оставляет пришедшие письма, остальные переспрашиваются по одному; не пришедшие UID
   записываются в `fetch_holes` и спрашиваются при следующем синке, даже ниже самого младшего UID в кэше.
...
3. **Пуш.** В приложении (`RustRepository.startBackgroundSync`): синк при старте, затем на каждый аккаунт цикл IMAP `IDLE` на INBOX
   (`wait_for_change`, перезаход каждые 10 минут, backoff 30 с → 5 мин при ошибках), периодический полный синк раз в 10 минут,
   и отправка outbox через 2 секунды после локального действия. У каждого аккаунта своя очередь синков; аккаунты идут
   параллельно.
...
fetch_holes(folder_id, uid)   -- v9: запрошено и не получено, спросить снова ниже «пола» дозагрузки
outbox: индекс (account_id, done); выполненные и откаченные строки удаляются -- v9
